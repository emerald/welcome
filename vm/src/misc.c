...
void OpoidsInit(void)
{
  ...
  if (OpNames == NULL) { 
    ...
    if (fstat(fd, &buf) < 0) {
      Warning("OpoidsInit can't stat opoid file:");
      buf.st_size = 65536;
    }
			     ...
			     void (*variable_f)(Object *, ConcreteType *), void (*variables_f)(int, u32 *))
{
  ...
  do {
	...
	if (lstate.sp > lstate.fp) {
	  ...
	  IFTRACE(memory, 1) {
	    if (base + sizeFromTemplate(theTemplate) < lstate.sp && 0) {
	      /*
	       * There is memory left over here that I don't have a template
	       * for, let's at least document it.
	       *
	       * This is not a big deal any more, because arguments to an
	       * invocation show up here, as does the extra spot for a create.
	       */
		unsigned char lastInstr = *(unsigned char *) (lstate.pc - 1);
		int wasCreate = lastInstr == CREATE;
	      TRACE(memory, 1,
