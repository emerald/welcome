...
char strBufs[4][16];
char *idStr(int id) {
    char *idStrBuf = strBufs[nextBuf++];
    if (nextBuf >= 4) nextBuf = 0;
    if (id == JNIL) {
	return "nil";
    } else {
...
char *ptrStr(int id) {
    char *ptrStrBuf = strBufs[nextBuf++];
    if (nextBuf >= 4) nextBuf = 0;
    if (id == JNIL) {
	return "nil";
    } else {
	sprintf(ptrStrBuf, "0x%08x", id);
	return ptrStrBuf;
    }
}
...
static void doLookAtEm(CheckpointData *theContents)
{
  ...
  for (i = 0; i < size; i++) {
	...
	TRACE(trans, 2, ("  name: \"%.*s\" id: %s", 
			 e->d.name->d.items, e->d.name->d.data, idStr(e->d.id)));
	if (e->d.code) {
	...
	TRACE(trans, 2, ("  id: %s name: \"%.*s\"", idStr(e->d.id), 
