...
void progressDistGC(void *arg)
{
  ...
  if (ISetSize(remotegreys) < 10 && ISetSize(interestingblacks) < 10) {
    IFTRACE(distgc, 3) {
      Object o;
      TRACE(distgc, 2, ("Greys:"));
      ISetForEach(remotegreys, o) {
	OID oid = OIDOf(o);
	TRACE(distgc, 3, ("%s -> %d", OIDString(oid), o));
      } ISetNext();
    }
    IFTRACE(distgc, 3) {
      Object o;
      TRACE(distgc, 2, ("Blacks:"));
      ISetForEach(interestingblacks, o) {
	OID oid = OIDOf(o);
	TRACE(distgc, 3, ("%s -> %d", OIDString(oid), o));
      } ISetNext();
    }
  ...
  for (nd = &allnodes->p; *nd; nd = &(*nd)->p) {
	...
	TRACE(distgc, 3, ("Skipping node with no object %s", NodeString((*nd)->srv)));
	continue;
    }
    if (! (*nd)->up) {
      TRACE(distgc, 3, ("Skipping down node %s", NodeString((*nd)->srv)));
      continue;
    }
    if (*nd == thisnode) {
      TRACE(distgc, 3, ("Skipping this node %s", NodeString((*nd)->srv)));
      continue;
    }
    TRACE(distgc, 3, ("Sending to node %s", NodeString((*nd)->srv)));
    if (ISetSize(remotegreys)) {
      TRACE(distgc, 3, ("Sending remote greys (%d) to %s",
			ISetSize(remotegreys), NodeString((*nd)->srv))); 
      str = prepareMsg(0, remotegreys);
      sendMsg((*nd)->srv, str);
    }
    if (ISetSize(interestingblacks)) {
      TRACE(distgc, 3, ("Sending interesting blacks (%d) to %s",
			ISetSize(interestingblacks), NodeString((*nd)->srv)));
  ...
  one.tv_sec = 0; one.tv_usec = 25000;
...
static void finishDistGC(u32 whichgc)
{
  ...
  TRACE(distgc, 2, ("%d marked states, %d unmarked states, %d marked, %d removed, %d builtins not removed",
...
void handleDistGCInfo(RemoteOpHeader *h, Node srv, Stream str)
{
  ...
  if (!inDistGC()) {
    extern int nDGCs;
    TRACE(distgc, 1, ("Starting dist gc #%d due to remote gc message",
		      nDGCs));
    startDistGC();
  }
  while (!AtEOF(str)) {
