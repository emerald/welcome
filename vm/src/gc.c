...
extern void gcollect_old(int force);
...
void gcollect(int force)
{
  ...
  assert(!inhibit_gc);
  nGCs++;
  TRACE(memory, 2, ("Starting new garbage collection #%d, remsetsize = %d", 
		    nGCs, remNum));
  IFTRACE(memory, 7) {
  ...
  TRACE(memory, 3, ("gcollect new live %d of %d words", liveWords(), wordsPerGeneration * (copyCount==1?1:2) * copyCount));
  if (old_end - nextGen <  wordsToBePromotedNextGC || force) {
    if (force) TRACE(memory, 2, ("Gcollect old because forced"));
    gcollect_old(force);
  }
...
void *gc_malloc_old (int lb, int remember)
{
  ...
  if (old_end - nextGen - wordsToBePromotedNextGC < lw) {
    if (inhibit_gc) {
      ...
      TRACE(memory, 3, ("Starting gcollect - out of old space"));
      gcollect(0);
      gcollect_old(0);
      if (old_end - nextGen -wordsToBePromotedNextGC < lw) {
	TRACE(memory, 0, ("old_end = %#x, next_gen = %#x, diff = %d, promo = %d\n",
...
void gcollect_old(int force)
{
  ...
  if (needDistGC() || force) {
    if (force) TRACE(memory, 2, ("Starting dist gc because forced"));
    startDistGC();
  }
}
